...
*/
    ...
    $query = "INSERT INTO purchase (uid,p_name,challan_no,si_no,gaddi_no,builty,sgst,cgst,tcs,lbr,total,transporter,due_days,nrtn,cash_amt,check_amt,inv_date,inv_no,upload_measurement,upload_hisab_copy,upload_builty,upload_extra,upload_invoice) VALUES
...
for($a=1;$a<=5;$a++)
{
    ...
    if($_POST['upload'][$index]=='measurement')
    {   
        $index++;
        $upload_measurement = $_FILES['measurement']['name'];
        $upload_measurement_tmp = $_FILES['measurement']['tmp_name'];
        $query.="'$upload_measurement',";
    }
    elseif($_POST['upload'][$index]=='hisab_copy')
    {
        $index++;
        $upload_hisab_copy = $_FILES['hisab_copy']['name'];
        $upload_hisab_copy_tmp = $_FILES['hisab_copy']['tmp_name'];
        $query.="'$upload_hisab_copy',";
    }
    elseif($_POST['upload'][$index]=='builty')
    {
        $index++;
        $upload_builty = $_FILES['builty']['name'];
    ...
    elseif($_POST['upload'][$index]=='extra')
    {
        $index++;
        $upload_extra = $_FILES['extra']['name'];
    ...
    elseif($_POST['upload'][$index]=='invoice')
    {
        $index++;
        $upload_invoice = $_FILES['invoice']['name'];
        $upload_invoice_tmp = $_FILES['invoice']['tmp_name'];
        $query.="'$upload_invoice'";
    }
    else
    {
        if($a==5)
 ...
 */
 ...
 foreach($_POST['upload'] as $i)
 {
    $index++;
    if($index < count($_POST['upload']))
    {
        $query.=",";
    }
    elseif($index == count($_POST['upload']))
    {
        $query.="";
    }
 ...
 */
 ...
 move_uploaded_file($tmp_upload_hisab_copy, 'uploads/' . $upload_measurement);
 move_uploaded_file($tmp_upload_builty, 'uploads/' . $upload_measurement);
 move_uploaded_file($tmp_upload_extra, 'uploads/' . $upload_measurement);
 move_uploaded_file($tmp_upload_invoice, 'uploads/' . $upload_measurement);
...
*/
    ...
    foreach ($m_type as $m_type2) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $query = "INSERT INTO product_item (inv_no,uid,order_id,p1,p2,p3,p4,p5,qty,m_type,cat_id,ptype,subtype)VALUES

 ('$inv_no','$uid','$lastid','$mt2','$mw2','$ml2','$cbt2','$cmt3','$qty2','$m_type2','$cat_id2','$ptype2','$subtype2')";
...
from products where cat_id='$cat_id2' and type='$ptype2' and subtype='$subtype2' and meserment_type='$m_type2'and mt='$mt2'and  mw='$mw2'and ml='$ml2' ORDER BY id DESC LIMIT 1";
        $rqss = $db->query($quss);
        //$countss=mysqli_num_rows($rqss); //or die(mysqli_error($db));
        //$rowqss=mysqli_fetch_array($rqss,MYSQLI_ASSOC);
        if (!$rqss || mysqli_num_rows($rqss)) {
            $query = "INSERT INTO products (p_code, cat_id,type, subtype,meserment_type,mt,mw,ml,cbt,cmt,qty)VALUES('','$cat_id2',' $ptype2','$subtype2','$measurment','$mt2','$mw2','$ml2','$cbt2','$cmt3','$qty2')";
